...
def check_venv_and_prompt() -> None:
    ...
    if not in_venv:
        ...
        print("   $ python3 bookstack_migrate.py --help")
        print()
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("bookstack_migrate.log", encoding="utf-8"),
    ],
)
...
class SimpleSession:
    ...
    def request(
        ...
        **kwargs: Any,
        ...
        req = urlrequest.Request(url, data=body, headers=req_headers, method=method)
        try:
            with urlrequest.urlopen(req, timeout=timeout) as resp:
                text = resp.read().decode("utf-8", errors="replace")
                return SimpleResponse(resp.getcode(), text, dict(resp.headers))
        except urlerror.HTTPError as exc:
            text = exc.read().decode("utf-8", errors="replace")
            return SimpleResponse(exc.code, text, dict(exc.headers))
        except urlerror.URLError as exc:
            raise BookStackError(f"Network error while calling {url}: {exc}") from exc
...
class BookStackClient:
    ...
    def _parse_json(self, resp: SimpleResponse) -> Dict[str, Any]:
        try:
            return resp.json()
        except json.JSONDecodeError as exc:
            raise BookStackError("Invalid JSON response", status=resp.status_code, body=resp.text) from exc

    def _request(self, method: str, path: str, **kwargs: Any) -> SimpleResponse:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                return resp
            except (BookStackError, Exception) as exc:
                last_exc = exc
                if attempt < max_retries:
                    time.sleep(backoff * (2 ** attempt))
                    continue
...
def fetch_openapi_spec(
    ...
    for url in candidates:
        try:
            resp = session.request("GET", url, timeout=DEFAULT_TIMEOUT)
            if resp.status_code >= 400:
...
class BackupOptions:
    ...
    dump_path: Path = Path("./bookstack.backup.sql")
    export_output: Path = Path("./dokuwiki_export")
    db: str = "bookstack"
    root_user: str = "root"
    root_password: str = ""
    host_port: int = 3307
    image: str = "mariadb:10.11"
    container_name: str = "bookstack-backup-temp"
    include_files: bool = False
    files_archive: Path = Path("./bookstack-files-backup.tar.gz")
        ...
        try:
            ...
            for url in list(urls)[:200]:
                ...
                if not target.exists():
                    if not _download_url(url, target, headers=client.session.headers, timeout=client.timeout):
                        continue
...
def cmd_export(options: ExportOptions) -> int:
    ...
    try:
        ...
        db_available = options.db is not None and options.user is not None and options.password is not None
        driver_name = None
        if db_available:
            try:
...
def _check_docker_available() -> None:
    if shutil.which("docker") is None:
        raise BookStackError("Docker is required for backup but was not found in PATH.")
...
def _find_datadir_candidates() -> List[Path]:
    """Locate candidate MySQL/MariaDB datadirs containing ibdata1."""
    bases = [
        Path.cwd(),
        Path.cwd().parent,
    ...
    for base in bases:
        for name in ("mysql", "mariadb", "data", "db", "mysql_data"):
            p = base / name
            if p.exists() and (p / "ibdata1").exists():
                resolved = p.resolve()
...
def _start_mariadb_container(options: BackupOptions) -> str:
    """Start a MariaDB container bound to the provided data directory."""
    datadir = options.datadir
    _stop_container(options.container_name)
    args = [
        "docker",
        "run",
        "-d",
    ...
    logger.info(f"Started MariaDB container {cid} from {datadir}")
    return cid


def _wait_for_mysql(container: str, user: str, password: str, timeout_seconds: int = 60) -> None:
    start = time.time()
    while time.time() - start < timeout_seconds:
        cmd = ["docker", "exec", container, "mysqladmin", "ping", f"-u{user}", "--protocol=TCP"]
        if password:
        ...
        if result.returncode == 0:
            return
        time.sleep(1)
    raise BookStackError("Timed out waiting for MariaDB to be ready inside container")


def _dump_database_from_container(
    container: str,
    db: str,
    user: str,
    password: str,
    ...
    cmd = ["docker", "exec", container, "mysqldump", f"-u{user}"]
    if password:
        cmd.append(f"-p{password}")
    cmd.append(db)
    with open(dump_path, "wb") as handle:
        res = subprocess.run(cmd, stdout=handle, stderr=subprocess.PIPE)
    if res.returncode != 0:
        msg = res.stderr.decode("utf-8", errors="replace").strip()
        raise BookStackError(f"mysqldump failed: {msg}")
    logger.info(f"Wrote database dump to {dump_path}")


def _archive_instance_files(instance_root: Path, archive_path: Path) -> Path:
    """Create a tar.gz of BookStack instance-specific files."""
    targets = [
        (instance_root / ".env", ".env"),
        (instance_root / "public" / "uploads", "public/uploads"),
        (instance_root / "storage" / "uploads", "storage/uploads"),
        (instance_root / "themes", "themes"),
    ]
    ...
    logger.info(f"Wrote files archive to {archive_path}")
    return archive_path


def cmd_backup(options: BackupOptions) -> int:
    """Spin up MariaDB from a data directory, dump it, and export to DokuWiki."""
    logger.info(f"Running backup command for datadir={options.datadir}")

    datadir = options.datadir
    if datadir is None:
        candidates = _find_datadir_candidates()
        if len(candidates) == 1:
            datadir = candidates[0]
            print(f"✅ Auto-detected datadir: {datadir}")
            logger.info(f"Auto-detected datadir: {datadir}")
        elif len(candidates) == 0:
            print("❌ Could not find a MySQL/MariaDB datadir (need ibdata1). Pass --datadir explicitly.")
            return 1
        else:
            print("❌ Multiple datadir candidates found. Pass --datadir explicitly:")
    ...
    if not datadir.exists():
        ...
        return 1
    if not (datadir / "ibdata1").exists():
        print(f"❌ Data directory does not look like a MySQL/MariaDB datadir: missing ibdata1 in {datadir}")
        return 1
    ...
    try:
        container_id = _start_mariadb_container(options)
        print(f"✅ Started MariaDB container ({options.container_name}) on port {options.host_port}")
        _wait_for_mysql(options.container_name, options.root_user, options.root_password)

        if not options.skip_dump:
            _dump_database_from_container(
                options.container_name,
                options.db,
                options.root_user,
            ...
            print(f"✅ Database dump written: {options.dump_path}")

        if options.include_files:
            archive_path = _archive_instance_files(options.instance_root, options.files_archive)
            print(f"✅ Files archive written: {archive_path}")

        if not options.skip_export:
            print("📤 Running DokuWiki export from temporary database...")
            export_opts = ExportOptions(
                db=options.db,
                user=options.root_user,
            ...
            code = cmd_export(export_opts)
            if code != 0:
                raise BookStackError(f"Export failed with exit code {code}")
        ...
        if container_id:
            _stop_container(options.container_name)
            logger.info(f"Stopped container {options.container_name}")
...
def get_db_driver(preferred: Optional[str] = None) -> Tuple[Optional[object], Optional[str]]:
    ...
    print("   Install one of the following, or use API mode:")
...
def load_driver(driver: str) -> Optional[Tuple[object, str]]:
    ...
    try:
        ...
        logger.warning(f"Database driver '{driver}' not available: {exc}")
        return None
...
def main() -> int:
    ...
    # Check venv only for export runs (avoid breaking help/version and automation).
    if (
        args.command == "export"
    ...
    if args.command == "backup":
        backup_opts = BackupOptions(
            ...
            files_archive=Path(args.files_archive),
            instance_root=Path(args.instance_root),
            skip_export=getattr(args, "skip_export", False),
            skip_dump=getattr(args, "skip_dump", False),
        )
        return cmd_backup(backup_opts)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    backup = sub.add_parser(
        "backup",
        help="Start MariaDB from a datadir, dump it, and run DokuWiki export",
    )
    backup.add_argument(
        "--datadir",
        required=False,
        help="Path to MySQL/MariaDB data directory (contains ibdata1)",
    )
    backup.add_argument(
        "--dump-path",
        default="./bookstack.backup.sql",
        help="Path to write logical database dump (mysqldump)",
    )
    backup.add_argument(
        "--export-output",
        default="./dokuwiki_export",
        help="Output directory for DokuWiki export",
    )
    backup.add_argument("--db", default="bookstack", help="Database name to dump")
    backup.add_argument("--root-user", default="root", help="Database user to connect with")
    backup.add_argument(
        "--root-password",
        default="",
        help="Database password for --root-user (empty allowed)",
    )
    backup.add_argument(
        "--host-port",
        type=int,
        default=3307,
        help="Host port to expose MariaDB during backup",
    )
    backup.add_argument(
        "--image",
        default="mariadb:10.11",
        help="MariaDB image tag to use for temporary container",
    )
    backup.add_argument(
        "--container-name",
        default="bookstack-backup-temp",
        help="Name for the temporary MariaDB container",
    )
    backup.add_argument(
        "--include-files",
        action="store_true",
        help="Also archive .env, public/uploads, storage/uploads, and themes",
    )
    backup.add_argument(
        "--files-archive",
        default="./bookstack-files-backup.tar.gz",
        help="Path to write files archive when --include-files is set",
    )
    backup.add_argument(
        "--instance-root",
        default=str(Path(__file__).resolve().parent.parent),
        help="BookStack root path for file backup",
    )
    backup.add_argument(
        "--skip-export",
        action="store_true",
        help="Skip DokuWiki export step",
    )
    backup.add_argument(
        "--skip-dump",
        action="store_true",
        help="Skip mysqldump step (still starts container for export)",
    )
